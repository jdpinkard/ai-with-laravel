...
<body class="antialiased text-lg">
        <div class="px-24 relative sm:flex sm:justify-center sm:items-center min-h-screen bg-dots-darker bg-center bg-gray-100 dark:bg-dots-lighter dark:bg-gray-900 selection:bg-red-500 selection:text-white">
            <div>
                <div class="space-y-8 dark:text-white selection:text-white">
                    @foreach($conversation?->messages ?? collect() as $message)
                    <div class="{{ $loop->last ? 'pending_response' : '' }}">
                        {{ $message->content }}
                    </div>
                    @endforeach
                </div>
